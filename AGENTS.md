...
- Frontend app: `client/src/app/` (Angular standalone components, signals, Angular Material, Tailwind). Pages: `admin-shell` (root-only, `/admin`), `event-manager-shell` (single-event ops, `/manager`), `voting-shell` (public voting, `/`); Hall of Fame lives in `client/src/app/components/hall-of-fame/` (`/hof`).
