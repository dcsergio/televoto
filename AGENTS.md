...
- `handleManageEvent`/`openManager` in `admin-shell.ts` navigate to `/manager` via the Angular `Router` in the *same* tab (not `window.open`), so root's `sessionStorage` token is simply still there — no opener/`noopener` concerns like the still-new-tab `/vote` and `/score` links.
