...
Televoto: web app for running live voting events (public/judge voting, admin panel, Hall of Fame final rankings). Frontend Angular + Angular Material (in `client/`); backend Express + Prisma (repo root); database PostgreSQL. UI text and server error messages are primarily **Italian** — keep new strings consistent with that.
